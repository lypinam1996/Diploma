...
public class AdminController
{
    ...
    public String addCorpus(Model model)
    {
        CorpusesEntity corpus = new CorpusesEntity();
        model.addAttribute("corpus", corpus);
        return "addCorpus";
    }

    @RequestMapping(value = "/addCorpus", method = RequestMethod.POST)
    public String saveCorpus(@ModelAttribute("corpus") CorpusesEntity corpus, Model model)
    {
        corpusService.saveObject(corpus);
        CorpusesEntity corpus1 = new CorpusesEntity();
        model.addAttribute("corpus", corpus1);
        model.addAttribute("successMessage", "Добавление прошло успешно");
        return "addFacts";
    }

    @RequestMapping(value = "/addFacts", method = RequestMethod.GET)
    public String addFacts(Model model)
    {
        CorpusesEntity corpus = corpusService.findMaxId();
        model.addAttribute("corpus", corpus);
        ClarifyingFactsEntity clarifyingFactsEntity = new ClarifyingFactsEntity();
        model.addAttribute("facts", clarifyingFactsEntity);
        ObjectsEntity object = objectService.findById(1);
        model.addAttribute("object", object);
    ...
    @RequestMapping(value = "/addFacts", method = RequestMethod.POST)
    public String SaveFacts(@ModelAttribute("facts") ClarifyingFactsEntity clarifyingFactsEntity, Model model)
    {
        clarifyingFacts.saveObject(clarifyingFactsEntity);
